...
class MediaResearcherAgent:
    ...
    async def research_async(self, media_outlet: str, journalist_name: str = None) -> Dict[str, Any]:
        """
        Async version: Research a media outlet and journalist.
        ...
        Example:
            >>> agent = MediaResearcherAgent(llm, search_tool)
            >>> result = await agent.research_async("TechCrunch", "Jane Smith")
        """
        # Perform search asynchronously
        raw_research = await self.search_tool.research_media_outlet_async(
            media_name=media_outlet,
            journalist_name=journalist_name
        )
